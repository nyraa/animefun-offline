...
## Config
### Multiple Thread
This program's downloading function is multiple-thread, by default max number of thread is 1, you can adjust it depending on your condition, **REMEMBER don't make over load to animefun**.

### Cookies
If you are normal user of animefun(normal account or haven't login), you need to wait the 30s advertisment(actually the time server counts is 25s) and the maximum resolution is 720p(no logined also can get 720p, you can just see 360p in webpage because just the UI higher 720p option).
